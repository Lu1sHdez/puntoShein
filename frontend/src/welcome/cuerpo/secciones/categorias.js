    ...
    const obtenerProductos = async () => {
      ...
      try {
        ...
        const filtrados = categoriaSeleccionada
          ? todos.filter((p) => p.subcategoria?.categoria?.id === categoriaSeleccionada)
          : todos;

        setProductos(filtrados.slice(0, 4)); // Solo mostrar los primeros 4
      } catch (error) {
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        {cargando ? (
          ...
          <div className="text-center text-gray-600 py-10">
            ...
            <p className="text-gray-500">No hay productos disponibles para esta categoría.<br />Prueba seleccionando otra.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-5">
            {productos.map((producto) => (
              <div key={producto.id} className="bg-white rounded-lg shadow hover:shadow-lg p-4 transition">
                <img
                  src={producto.imagen}
                  alt={producto.nombre}
