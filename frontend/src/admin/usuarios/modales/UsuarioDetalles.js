// src/admin/usuarios/modales/ModalDetalleUsuario.js
...
const ModalDetalleUsuario = ({ visible, userId, onClose }) => {
  ...
  return createPortal(
    <div className="fixed inset-0 bg-black bg-opacity-40 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="bg-white p-6 rounded-xl shadow-xl w-[90%] max-w-3xl h-[80vh] overflow-y-auto relative">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-8 text-gray-700 text-base">
            <div><strong>Nombre de Usuario:</strong> {usuario.nombre_usuario}</div>
            <div><strong>Nombre:</strong> {usuario.nombre}</div>
            <div><strong>Apellido Paterno:</strong> {usuario.apellido_paterno}</div>
            <div><strong>Apellido Materno:</strong> {usuario.apellido_materno}</div>
            <div><strong>Teléfono:</strong> {usuario.telefono}</div>
            <div><strong>Correo Electrónico:</strong> {usuario.correo}</div>
